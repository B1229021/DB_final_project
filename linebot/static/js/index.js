const API_BASE_URL = 'https://a85e-2404-0-8523-a4e7-3050-1d5e-13bd-bf24.ngrok-free.app/linebot3.0'; // 你的ngrok網址
...
async function loadCategories(retry = 5) {
    try {
        const resp = await fetch(`${API_BASE_URL}/action.php?action=list_categories&_t=${Date.now()}`);
        const contentType = resp.headers.get("content-type");
        const text = await resp.text();
        // Debug log
        console.log('伺服器原始回應：', text);
        ...
        if (resp.ok && contentType && contentType.includes("application/json")) {
            ...
            throw new Error("伺服器未回傳 JSON，內容可能是錯誤頁或 ngrok 斷線: " + text.substring(0, 120));
        }
    } catch (err) {
        if (retry > 0) {
...
async function loadEventTypes() {
    const resp = await fetch(`${API_BASE_URL}/action.php?action=list_event_types`);
...
async function loadUsers() {
    const resp = await fetch(`${API_BASE_URL}/action.php?action=list_users`);
...
async function loadEvents(catId = '', time = '') {
    ...
    let url = `${API_BASE_URL}/action.php?action=list_events`;
    let params = [];
...
window.showEventDetail = async function(orderid) {
    const resp = await fetch(`${API_BASE_URL}/action.php?action=event_detail&orderid=`+orderid);
...
async function joinEvent(orderid) {
    const resp = await fetch(`${API_BASE_URL}/action.php?action=join_event`, {
        method: 'POST',
...
async function leaveEvent(orderid) {
    const resp = await fetch(`${API_BASE_URL}/action.php?action=leave_event`, {
        method: 'POST',
        headers: {'Content-Type':'application/x-www-form-urlencoded'},
        body: `orderid=${orderid}&uid=${CURRENT_UID}`
    });
...
async function cancelEvent(orderid) {
    ...
    const resp = await fetch(`${API_BASE_URL}/action.php?action=cancel_event`, {
        method: 'POST',
        headers: {'Content-Type':'application/x-www-form-urlencoded'},
        body: `orderid=${orderid}&uid=${CURRENT_UID}`
    });
...
async function endEvent(orderid) {
    ...
    const resp = await fetch(`${API_BASE_URL}/action.php?action=end_event`, {
        method: 'POST',
        headers: {'Content-Type':'application/x-www-form-urlencoded'},
        body: `orderid=${orderid}&uid=${CURRENT_UID}`
    });
...
document.getElementById('historyBtn').onclick = async function() {
    ...
    const resp = await fetch(`${API_BASE_URL}/action.php?action=list_my_events&uid=${CURRENT_UID}`);
    const events = await resp.json();
    // 渲染歷史紀錄
...
document.getElementById('createForm').onsubmit = async function(e) {
    ...
    const resp = await fetch(`${API_BASE_URL}/action.php?action=create_event`, {
        method: 'POST',
        body: data
    });
    const result = await resp.json();
