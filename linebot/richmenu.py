...
# 設定 LINE Bot API
NGROK_URL = 'https://1868-60-250-225-145.ngrok-free.app'  # 你的 ngrok 網址
...
user_rich_menu = RichMenu(
    ...
    areas=[
        ...
        RichMenuArea(
            ...
            action=URIAction(label='登入', uri=f'{NGROK_URL}/index')
        ),
        #評分
        RichMenuArea(
        ...
        RichMenuArea(
            ...
            action=URIAction(label='新增', uri=f'{NGROK_URL}/index')
        ),
        #刪除
        RichMenuArea(
            bounds=RichMenuBounds(x=1250, y=843, width=1250, height=843),
            action=URIAction(label='刪除', uri=f'{NGROK_URL}/grade')
        )
    ]
)
